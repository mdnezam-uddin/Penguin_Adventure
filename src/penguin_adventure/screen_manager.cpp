...
#include "penguin_adventure/controller_hud.hpp"
...
struct ScreenManager::FPS_Stats
{
  ...
  void report_frame()
  {
    ...
    int dtime_us = static_cast<int>(std::chrono::duration_cast<
                                        std::chrono::microseconds>(time_now - time_prev)
                                        .count());
...
ScreenManager::ScreenManager(VideoSystem &video_system, InputManager &input_manager) : m_video_system(video_system),
...
void ScreenManager::push_screen(std::unique_ptr<Screen> screen, std::unique_ptr<ScreenFade> screen_fade)
{
...
void ScreenManager::push_screen(std::function<Screen *()> callback, std::unique_ptr<ScreenFade> screen_fade)
{
...
void ScreenManager::pop_screen(std::unique_ptr<ScreenFade> screen_fade)
{
...
void ScreenManager::set_screen_fade(std::unique_ptr<ScreenFade> screen_fade)
{
...
void ScreenManager::quit(std::unique_ptr<ScreenFade> screen_fade)
{
...
void ScreenManager::on_window_resize()
{
...
void ScreenManager::draw_fps(DrawingContext &context, FPS_Stats &fps_statistics)
{
...
void ScreenManager::draw_player_pos(DrawingContext &context)
{
...
void ScreenManager::draw(Compositor &compositor, FPS_Stats &fps_statistics)
{
    ...
    if (Console::current()->hasFocus() &&
        ((event.type == SDL_MOUSEWHEEL && LOGMOUSEY(event.wheel.mouseY) < Console::HEIGHT) ||
         ((event.type == SDL_MOUSEBUTTONDOWN ||
          ...
          LOGMOUSEY(event.button.y) < Console::HEIGHT) ||
...
#undef LOGMOUSEY
    ...
    case SDL_KEYDOWN:
      ...
      // Stage shortcuts (F5/F6/F7) - only in developer mode or editor testing
      else if ((g_config->developer_mode || (Editor::current() && Editor::current()->is_testing_level())) &&
               session != nullptr && session->get_stage_manager())
      {
        if (event.key.keysym.sym == SDLK_F5)
        {
          session->get_stage_manager()->force_set_stage(StageManager::Stage::STAGE_1);
        }
        else if (event.key.keysym.sym == SDLK_F6)
        {
          session->get_stage_manager()->force_set_stage(StageManager::Stage::STAGE_2);
        }
        else if (event.key.keysym.sym == SDLK_F7)
        {
          session->get_stage_manager()->force_set_stage(StageManager::Stage::STAGE_3);
        }
      }
      break;
...
bool ScreenManager::has_pending_fadeout() const
{
...
void ScreenManager::loop_iter()
{
  ...
  if (((steps > 0 && !m_screen_stack.empty()) || always_draw) && m_actions.empty() || m_screen_fade)
  {
    // Draw a frame
...
#ifdef EMSCRIPTEN
  EM_ASM({ supertux2_syncfs(); }, 0); // EM_ASM is a variadic macro and Clang requires at least 1 value for the variadic argument
#endif
}
