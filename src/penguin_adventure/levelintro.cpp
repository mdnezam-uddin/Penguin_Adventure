//  Penguin Adventure -- LevelIntro screen
...
#include "penguin_adventure/levelintro.hpp"
...
LevelIntro::LevelIntro(const Level &level, const Statistics *best_level_statistics, const PlayerStatus &player_status) : m_level(level),
...
void LevelIntro::setup()
{
}

void LevelIntro::update(float dt_sec, const Controller &controller)
{
  ...
  for (int i = 0; i < InputManager::current()->get_num_users(); i++)
  {
    if (!InputManager::current()->has_corresponsing_controller(i) && !InputManager::current()->m_uses_keyboard[i] && i != 0)
      continue;
...
void LevelIntro::draw(Compositor &compositor)
{
  ...
  if ((!author.empty()) && (author != "Penguin Adventure Team"))
  {
    context.color().draw_center_text(Resources::small_font, author, Vector(0, static_cast<float>(py)), LAYER_FOREGROUND1, s_author_color);
  ...
  for (int i = 0; i < static_cast<int>(m_player_sprite.size()); i++)
  {
    ...
    m_player_sprite[i]->draw(context.color(), Vector((context.get_width() - m_player_sprite[i]->get_current_hitbox_width()) / 2 - offset, static_cast<float>(py) + m_player_sprite_py[i] - m_player_sprite[i]->get_current_hitbox_height()), LAYER_FOREGROUND1);

    Color power_color = (m_player_status.bonus[i] == BONUS_FIRE ? Color(1.f, 0.7f, 0.5f) : m_player_status.bonus[i] == BONUS_ICE ? Color(0.7f, 1.f, 1.f)
