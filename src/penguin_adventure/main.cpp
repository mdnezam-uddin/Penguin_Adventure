...
#include "penguin_adventure/command_line_arguments.hpp"
...
#include "penguin_adventure/menu/download_dialog.hpp"
...
Main::Main() :
  ...
  m_screen_manager(),
  m_savegame(),
  m_downloader(), // Used for getting the version of the latest Penguin Adventure release.
  m_version_info()
{
}
...
void PhysfsSubsystem::find_mount_datadir()
{
  ...
  else if (const char* env_datadir = getenv("PENGUIN_ADVENTURE2_DATA_DIR"))
  {
...
void PhysfsSubsystem::find_mount_userdir()
{
  ...
  else if (const char* env_userdir = getenv("PENGUIN_ADVENTURE2_USER_DIR"))
  {
    m_userdir = env_userdir;
  }
  else
  {
    m_userdir = PHYSFS_getPrefDir("Penguin Adventure","supertux2");
  }
...
if (FileSystem::is_directory(olduserdir)) {
  ...
  if (!FileSystem::is_directory(m_userdir))
  {
    ...
    log_info << "Created Penguin Adventure userdir: " << m_userdir << std::endl;
  }
...
Main::init_video()
{
  VideoSystem::current()->set_title("Penguin Adventure " PACKAGE_VERSION);

  const char* icon_fname =
#ifdef IS_PENGUIN_ADVENTURE_RELEASE
...
Main::run(int argc, char** argv)
{
  ...
  init_emscripten();
	...
	std::string prefpath = SDL_GetPrefPath("Penguin Adventure", "supertux2");
...
Main::release_check()
{
  ...
  // Detect a potential new release of Penguin Adventure. If a release, other than
  ...
  TransferStatusPtr status = m_downloader->request_string_download(
    "https://raw.githubusercontent.com/Penguin Adventure/addons/master/ver_info.nfo",
    m_version_info
  );
  status->then([this, status](bool success)
  {
    ...
    if (mapping.get("latest", latest_ver) && latest_ver != PACKAGE_VERSION_TAG)
    {
      ...
      if (version != latest_ver)
      {
        ...
        notif->set_text(fmt::format(fmt::runtime(_("New release: Penguin Adventure v{}!")), latest_ver));
        notif->set_mini_text(_("Click for more details."));
        notif->on_press([latest_ver]()
                       {
                         Dialog::show_confirmation(fmt::format(fmt::runtime(_("A new release of Penguin Adventure (v{}) is available!\nFor more information, you can visit the Penguin Adventure website.\n\nDo you want to visit the website now?")), latest_ver), []()
                                                   {
