...
#include "penguin_adventure/menu/options_menu.hpp"
...
OptionsMenu::refresh()
{
  ...
  switch (m_type) // Insert label and menu items, appropriate for the chosen OptionsMenu type
  {
    ...
    case EXTRAS: /** EXTRAS */
    {
      ...
      .set_help(_("Manage whether Penguin Adventure should display the levels you play on your social media profiles (Discord)"));
    ...
    defined(__OPENBSD) || defined(__NetBSD) && !defined(STEAM_BUILD)
      ...
      if (!g_config->disable_network)
        add_toggle(MNID_RELEASE_CHECK, _("Check for new releases"), &g_config->do_release_check)
          .set_help(_("Allows the game to perform checks for new Penguin Adventure releases on startup and notify if any found."));
#endif
...
#pragma GCC diagnostic pop
        ...
        if (!resultds)
        {
          Dialog::show_message(_("The game couldn't detect your browser resolution.\n"
                                 ...
                                 "in the Penguin Adventure custom HTML template.\n"));
        }
      }
      break;
#endif
