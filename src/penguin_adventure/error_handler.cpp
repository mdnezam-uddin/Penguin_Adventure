...
{
#ifdef WIN32
  // Adapted from Penguin AdventureKart, (C) 2013-2015 Lionel Fuentes, GPLv3
...
{
  char msg[] = "Penguin Adventure has encountered an unrecoverable error!";
...
ErrorHandler::error_dialog_exception(const std::string& exception)
{
  ...
  stream << "Penguin Adventure has encountered a fatal exception!";
  ...
  if (resultbtn == 0)
  {
    ...
    stream << SDL_GetPrefPath("Penguin Adventure", "supertux2")
           << "/console.err";
...
ErrorHandler::report_error(const std::string& details)
{
  ...
  url << "https://github.com/penguin_adventure/penguin_adventure/issues/new"
         "?template=crash.yml"
