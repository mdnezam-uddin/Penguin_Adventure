...
SDLBaseVideoSystem::SDLBaseVideoSystem() : m_sdl_window(nullptr, &SDL_DestroyWindow),
                                           m_desktop_size(),
...
void SDLBaseVideoSystem::set_title(const std::string &title)
{
  SDL_SetWindowTitle(m_sdl_window.get(), title.c_str());
}

void SDLBaseVideoSystem::set_icon(const SDL_Surface &icon)
{
  SDL_SetWindowIcon(m_sdl_window.get(), const_cast<SDL_Surface *>(&icon));
}

Size SDLBaseVideoSystem::get_window_size() const
{
  Size size;
...
void SDLBaseVideoSystem::on_resize(int w, int h)
{
  g_config->window_size = Size(w, h);
  apply_config();
}

void SDLBaseVideoSystem::create_sdl_window(Uint32 flags)
{
...
#warning Android needs SDL_HINT_MOUSE_TOUCH_EVENTS to work properly, but the   \
  ...
  m_sdl_window.reset(SDL_CreateWindow("Penguin Adventure",
  ...
  if (g_config->fit_window)
  {
    EM_ASM({
      if (window.supertux_setAutofit)
        window.supertux_setAutofit(true); }, 0); // EM_ASM is a variadic macro and Clang requires at least 1 value for the variadic argument
  }
#endif
}

void SDLBaseVideoSystem::apply_video_mode()
{
